...
const s=window.supabase.createClient(U,K,{auth:{persistSession:false,autoRefreshToken:false}});
...
async function upload(){
 ...
 for(const f of files){const safe=f.name.replace(/[^\u4e00-\u9fa5\w.()\- ]/g,'_');const path=Date.now()+'_'+Math.random().toString(36).slice(2,8)+'_'+safe;const {error}=await s.storage.from(B).upload(path,f,{upsert:false,contentType:f.type||'application/octet-stream'});if(error){status.textContent='上传失败：'+error.message;alert('文件上传失败：'+error.message);btn.disabled=false;return}}
